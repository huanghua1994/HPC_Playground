    ...
    const double *X, const int ldX, double *Y, const int ldY)
{
    ...
    if (CSRP->first_row[iblock] == CSRP->last_row[iblock])
    {
        ...
        CSR_SpMM_RM_row_seg(
            ...
            X, ldX, CSRP->fr_res + iblock, nblk
        );
        
        for (int icol = 0; icol < ncol; icol++)
        {
            CSRP->lr_res[icol * nblk + iblock] = 0.0;
...
{
    assert(ldX >= CSRP->ncol);
    assert(ldY >= CSRP->nrow);
    ...
    if (ncol > CSRP->X_ncol)
    {
        ...
        free(CSRP->lr_res);
        CSRP->X_ncol = ncol;
        CSRP->fr_res = (double*) malloc(sizeof(double) * CSRP->X_ncol * nblk);
        CSRP->lr_res = (double*) malloc(sizeof(double) * CSRP->X_ncol * nblk);
    ...
    for (int icol = 0; icol < ncol; icol++)
    {
        ...
        double *fr_res_i = CSRP->fr_res + icol * nblk;
        double *lr_res_i = CSRP->lr_res + icol * nblk;
        for (int iblock = 0; iblock < nblk; iblock++)
        {
            if (CSRP->fr_intact[iblock] == 0)
            {
