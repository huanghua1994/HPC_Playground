...
static void partitionBlocks(const int nelem, const int nblk, int *displs)
{
    int bs0 = nelem / nblk;
    int bs1 = bs0;
    int remainder = nelem % nblk;
    if (remainder > 0) bs1++;
    displs[0] = 0;
    for (int i = 0; i < remainder; i++)
        displs[i + 1] = displs[i] + bs1;
    for (int i = remainder; i < nblk; i++)
...
{
    CSRPlusMatrix_t _CSRP = (CSRPlusMatrix_t) malloc(sizeof(CSRPlusMatrix_s));
    
    _CSRP->nrow   = nrow;
    _CSRP->ncol   = ncol;
    _CSRP->nnz     = nnz;
    _CSRP->row_ptr = (int*)    malloc(sizeof(int)    * (nrow + 1));
    ...
    compressIndices(_row, _CSRP->row_ptr, nrow, nnz);
...
void CSRP_partition(const int nblk, CSRPlusMatrix_t CSRP)
{
    CSRP->X_ncol    = 1;
    CSRP->nblk      = nblk;
    CSRP->nnz_spos  = (int*)    malloc(sizeof(int)    * nblk);
    CSRP->nnz_epos  = (int*)    malloc(sizeof(int)    * nblk);
    CSRP->first_row = (int*)    malloc(sizeof(int)    * nblk);
    CSRP->last_row  = (int*)    malloc(sizeof(int)    * nblk);
    CSRP->fr_intact = (int*)    malloc(sizeof(int)    * nblk);
    CSRP->lr_intact = (int*)    malloc(sizeof(int)    * nblk);
    CSRP->fr_res    = (double*) malloc(sizeof(double) * nblk);
    CSRP->lr_res    = (double*) malloc(sizeof(double) * nblk);
    ...
    partitionBlocks(nnz, nblk, nnz_displs);
    
    for (int iblock = 0; iblock < nblk; iblock++)
    {
        int block_nnz_spos = nnz_displs[iblock];
        int block_nnz_epos = nnz_displs[iblock + 1] - 1;
        int spos_in_row = lower_bound(row_ptr, nrow + 1, block_nnz_spos);
        int epos_in_row = lower_bound(row_ptr, nrow + 1, block_nnz_epos);
    ...
    CSRP->last_row[nblk - 1] = nrow - 1;
    CSRP->nnz_epos[nblk - 1] = row_ptr[nrow] - 1;
