...
//   need_symm : If the input matrix needs to be symmetric
// Output parameters:
//   *nrow_, *ncol_, *nnz_  : Number of rows, columns, and nonzeros
//   **row_, **col_, **val_ : Row indices, column indices, and values
int mm_read_sparse_RPI(const char *fname, const int need_symm, int *nrow_, int *ncol_, int *nnz_, int **row_, int **col_, double **val_);
