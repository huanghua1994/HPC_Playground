function multigrid_test(gridsizes, BCs)
...
while (Nx > coarse_size && Ny > coarse_size && Nz > coarse_size)
  [P Nx1 Ny1 Nz1] = prolong(Nx, Ny, Nz, BCs);
  R = 0.125*P';
  ...
  %Alow = GenDiscreteLaplacian(cellDims, [Nx Ny Nz], latVecs, BCs, FDn);
  A = R*A*P;
  mg.A{level+1} = A;
...
if norm(BCs) == 0
  % if all periodic boundaries, use pseudoinverse for coarsest level solve
  mg.pinv = pinv(full(mg.A{mg.numlevels}));
  mg.usepinv = 1;
else
  % save cholesky factors for coarsest level solve
  mg.chol = chol(-mg.A{mg.numlevels}, 'lower');
  % note sign since our matrix is negative definite
  mg.usepinv = 0;
end
...
for k = 1:40
  ...
  if mg.usepinv == 1
    e{level+1} = mg.pinv * r{mg.numlevels};
  else
