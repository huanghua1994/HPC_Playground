function Lap = DiscreteLaplacian3D(L1, L2, L3, Nx, Ny, Nz, FDn, BC)
% @brief    Discretize the Laplacian operator in a cuboid domain with 
%           homogeneous Dirichlet / periodic boundary condition
%           and returns the matrix Lap.
%
% @param BC    Boundary condition: 1--zero boundary; 2--periodic boundary
...
% @2016-2019 Georgia Institute of Technology.

if (L1 <= 0)
    error('DiscreteLaplacian(S): ''L1'' must be positive')
elseif (L2 <= 0)
    error('DiscreteLaplacian(S): ''L2'' must be positive')
elseif (L3 <= 0)
    error('DiscreteLaplacian(S): ''L3'' must be positive')
elseif ((Nx - round(Nx) ~= 0)||(Nx <= 0))
    error('DiscreteLaplacian(S): ''Nx'' must be a positive integer')
elseif ((Ny - round(Ny) ~= 0)||(Ny <= 0))
    error('DiscreteLaplacian(S): ''Ny'' must be a positive integer')
elseif ((Nz - round(Nz) ~= 0)||(Nz <= 0))
    error('DiscreteLaplacian(S): ''Nz'' must be a positive integer')
elseif ((FDn - round(FDn) ~= 0)||(FDn <= 0))
    error('DiscreteLaplacian(S): ''FDn'' must be a positive integer')    
end
...
nnzCount = (3 * (2 * FDn) + 1) * N;

% Row and column indices and the corresponding non-zero values
% used to generate sparse matrix Lap s.t. Lap(I(k),J(k)) = V(k)
I = zeros(nnzCount,1);
J = zeros(nnzCount,1);
V = zeros(nnzCount,1);
